<template>
  <div class="flex items-center justify-between py-3 px-6 mx-8">
    <div class="flex flex-1 items-center justify-between">
      <div>
        <p class="text-sm text-gray-700">
          ...
          to
          {{ ' ' }}
          <span class="font-medium">{{ Math.min(result.totalResults, (result.page + 1) * result.limit) }} </span>
          {{ ' ' }}
          of
          {{ ' ' }}
      ...
      <div>
        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
          ...
          <a 
              ...
              :class="{ 
                  'text-gray-400': (result.page + 1) === result.totalPages,
                  'text-gray-800': (result.page + 1) !== result.totalPages,
                  'pointer-events-none': (result.page + 1) === result.totalPages 
              }"
              @click="pageSelectHandler(result.page + 1)"
          >Next
          </a>
...
defineProps<{
    result: PaginationResult<SerializeObject<Record<string, any>>>,
