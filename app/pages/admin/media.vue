<template>
    <div>
        ...
        <UTable
            ...
            class="min-h-96"
        >
            <template #lastModified-cell="{ row }: { row: TableRow<S3Object> }">
                <span>
                    {{ transformDateTime(row.getValue('lastModified')) }}
                </span>
            </template>

            <template #name-cell="{ row }: { row: TableRow<S3Object> }">
                <button
                    class="text-website-off-black cursor-pointer underline"
                    @click="() => handleNameClick(row.original)"
                >
                    {{ row.getValue('name') }}
                </button>
            </template>

            <template #size-cell="{ row }: { row: TableRow<S3Object> }">
                <span>
                    {{ formatBytes(row.getValue('size')) }}
                </span>
            </template>

            <template #action-cell="{ row }: { row: TableRow<S3Object> }">
                <div class="flex flex-row justify-end">
                    <ClipboardIcon
                        v-if="!row.original.isFolder"
                        class="mr-4 h-6 w-6 cursor-pointer"
                        fill="gray"
                        @click="() => copyToClipboard(row.getValue('name'))"
                    />
                    <TrashIcon
                        v-if="currentFolder.length > 0"
...
<script lang="ts" setup>
    ...
    import type { S3Object } from '~~/shared/lib/aws';
    ...
    const selectedObject: Ref<S3Object | null> = ref(null);

    const files: Ref<S3Object[]> = ref([]);
    ...
    const handleNameClick = (row: S3Object) => {
        if (row.isFolder) {
    ...
    function createUppy() {
        ...
        uppy = new Uppy({}).use(Dashboard).use(AwsS3, {
            ...
            shouldUseMultipart: (file) =>
                file?.size ? file!.size > 100 * MiB : false,
